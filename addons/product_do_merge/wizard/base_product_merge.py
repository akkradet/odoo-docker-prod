...
class MergeProductAutomatic(models.TransientModel):
    ...
    group_by_name = fields.Boolean("Template")
    ...
    product_ids = fields.Many2many(
        "product.product", "product_rel", "product_merge_id", "product_id", string="Products to merge"
    )
    ...
    def _update_foreign_keys(self, src_products, dst_product):
        ...
        """
        _logger.debug(
            "_update_foreign_keys for dst_product: %s for src_products: %s", dst_product.id, str(src_products.ids)
        )
    ...
    def _update_reference_fields(self, src_products, dst_product):
        ...
        """
        _logger.debug(
            "_update_reference_fields for dst_product: %s for src_products: %r", dst_product.id, src_products.ids
        )
    ...
    def _update_values(self, src_products, dst_product):
        ...
        if parent_id and parent_id != dst_product.id:
            try:
                ...
                _logger.info(
                    "Skip recursive product hierarchies for parent_id %s of product: %s", parent_id, dst_product.id
                )
    ...
    def _merge(self, product_ids, dst_product=None):
        ...
        if (
            ...
            .sudo()
            .search([("product_id", "in", [product.id for product in src_products])])
        ):
            raise UserError(
                _(
        ...
        dst_product.message_post(
            body="%s %s"
            % (
                _("Merged with the following products:"),
                ", ".join("%s  (ID %s)" % (p.name, p.id) for p in src_products),
            )
        )
    ...
    def _action_next_screen(self):
        ...
        if self.line_ids:
            ...
            values.update(
                {"current_line_id": False, "product_ids": [], "state": "finished",}
            )
    ...
    def _process_query(self, query):
        ...
        for min_id, aggr_ids in self._cr.fetchall():
            ...
            self.env["base.product.merge.line"].create(
                {"wizard_id": self.id, "min_id": min_id, "aggr_ids": products.ids,}
            )
            counter += 1
